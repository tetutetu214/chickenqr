...
### 2026-05-23: QR と画像のピクセル粒度を揃える設計（試行錯誤の記録）
- 「ニワトリのドットと QR のドットの大きさが違ってダメ」というフィードバックへの根本対応
- 仕組み: QR モジュール (1 ドット) サイズと画像のピクセル粒サイズを「整数比」で揃えると視覚的に揃って見える
- 試行錯誤:
  - 第1試行: 画像を 32x32 NEAREST 縮小 → ニワトリの細い線が欠ける (元画像は 34x34 ピクセルアートだった)
  - 第2試行: 14/16/20/32 を試作 → 14-20 は絵が崩壊、32 が認識可能
  - 第3試行: 33/34/36/40/48 を試作 → 34 が「線が落ちない最小解像度」
- 最終設計:
  - QR `width: 340` + `typeNumber: 4` (33 モジュール) → 1 モジュール = 10.3 px
  - 画像 34x34 → 170x170 px NEAREST (5x) → 1 粒 = 5 px
  - **粒度比 5:10.3 ≈ 1:2 整数比**
  - styles.css に `image-rendering: pixelated; image-rendering: crisp-edges` で表示時の補完無効化
- **教訓**: ピクセルアート系画像を埋め込むときは「元画像の本来のドット解像度」を試作で見極める。Pillow の anti-alias 残骸でメジャーラン測定は失敗する → 複数解像度プレビューを目視比較するのが確実

### 2026-05-23: QR 読めない事故と仕組みの正しい理解
- imageSize=0.6 + hideBackgroundDots=false まで攻めたら QR が読めなくなった
- 「中央に画像を置いて周りに QR データ」という構造ではなく、QR は格子全体にデータ+誤り訂正コードが分布し、画像で隠れたドットを誤り訂正で復元する仕組み
- 誤り訂正レベル H = 最大 30% 復元。imageSize 0.5 (面積 25%) が安全圏
- hideBackgroundDots=false にすると画像下のドットも描画され、輪郭線が QR ドットを不規則に上書きしてデータ破壊につながる
- **教訓**: ロゴ入り QR は「画像で 30% 以内のドットを隠す + 誤り訂正で復元」が成立条件。仕様推奨値 (imageSize 0.4, hideBackgroundDots true) を超える場合は実機読み取りテスト必須
